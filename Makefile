...
pkgdatadir = $(datadir)/hash
pkgincludedir = $(includedir)/hash
pkglibdir = $(libdir)/hash
pkglibexecdir = $(libexecdir)/hash
...
DIST_COMMON = $(srcdir)/Makefile.in $(srcdir)/Makefile.am \
	...
	$(top_srcdir)/src/config/hash-config.h.in \
	...
	$(dist_noinst_SCRIPTS) COPYING INSTALL build-aux/config.guess \
...
CONFIG_HEADER = $(top_builddir)/src/config/hash-config.h
...
ACLOCAL = ${SHELL} /root/compile/hash/testnet/build-aux/missing aclocal-1.13
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /root/compile/hash/testnet/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /root/compile/hash/testnet/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /root/compile/hash/testnet/build-aux/missing automake-1.13
AWK = gawk
BDB_CPPFLAGS =  -I/usr/include/libdb4/
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = hash-cli
BITCOIN_DAEMON_NAME = hashd
BITCOIN_GUI_NAME = hash-qt
BITCOIN_TX_NAME = hash-tx
BOOST_CHRONO_LIB = -lboost_chrono-mt
BOOST_CPPFLAGS = -pthread -I/usr/include
BOOST_FILESYSTEM_LIB = -lboost_filesystem
BOOST_LDFLAGS = -L/usr/lib64
BOOST_LIBS = -L/usr/lib64 -lboost_system -lboost_filesystem -lboost_program_options-mt -lboost_thread -lboost_chrono-mt
...
LUPDATE = 
MAINT = 
MAKEINFO = ${SHELL} /root/compile/hash/testnet/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
MINIUPNPC_LIBS = 
...
OTOOL64 = 
PACKAGE = hash
PACKAGE_BUGREPORT = www.hashplatform.org
PACKAGE_NAME = Hash Core
PACKAGE_STRING = Hash Core 1.5.1
PACKAGE_TARNAME = hash
PACKAGE_URL = 
...
ZMQ_CFLAGS =  
ZMQ_LIBS = -lzmq  
abs_builddir = /root/compile/hash/testnet
abs_srcdir = /root/compile/hash/testnet
abs_top_builddir = /root/compile/hash/testnet
abs_top_srcdir = /root/compile/hash/testnet
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /root/compile/hash/testnet/build-aux/install-sh
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_hash_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info \
  leveldb_baseline_filtered.info test_hash_coverage.info test_hash.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/Hash-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/hash-config.h: src/config/stamp-h1
	@if test ! -f $@; then rm -f src/config/stamp-h1; else :; fi
	@if test ! -f $@; then $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1; else :; fi

src/config/stamp-h1: $(top_srcdir)/src/config/hash-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/hash-config.h
$(top_srcdir)/src/config/hash-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/hash-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/Hash-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
#	$(PYTHON) $(OSX_DEPLOY_SCRIPT) $(OSX_APP) -add-qt-tr $(OSX_QT_TRANSLATIONS) -translations-dir=$(QT_TRANSLATION_DIR) -dmg -fancy $(OSX_FANCY_PLIST) -verbose 2 -volname $(OSX_VOLNAME) -add-resources contrib/macdeploy/libboost_system-mt.dylib
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Hash-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Hash-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_hash.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_hash -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_hash_filtered.info: test_hash.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_hash_filtered.info
#	$(MKDIR_P) qa/tmp
...
#test_hash_coverage.info: baseline_filtered_combined.info test_hash_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_hash_filtered.info -o $@

#total_coverage.info:  baseline_filtered_combined.info test_hash_filtered.info block_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_hash_filtered.info -a block_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_hash.coverage/.dirstamp:  test_hash_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_hash.coverage/.dirstamp total.coverage/.dirstamp

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf test_hash.coverage/ total.coverage/ $(OSX_APP)
